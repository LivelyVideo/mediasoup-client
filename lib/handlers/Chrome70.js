...
class Handler extends EnhancedEventEmitter
{
	constructor(direction, rtpParametersByKind, settings)
	{
		...
		this._transportOptions = direction === 'send' ? settings.sendTransportOptions : settings.recvTransportOptions;
		const is = this._transportOptions.udp !== false ? (settings.turnServers || []) : [];
		...
		return Promise.resolve()
			.then(() =>
			{
				...
				if (producer.simulcast) {
					console.log('asdf simulcast')
					transceiver = this._pc.addTransceiver(track, { 
						direction: 'sendonly', 
						sendEncodings: [
							{ rid: "h", active: true, maxBitrate: 5000000 },
							{ rid: "m", active: true, maxBitrate: 1000000, scaleResolutionDownBy: 2 },
							{ rid: "l", active: true, maxBitrate: 300000, scaleResolutionDownBy: 4 }
						]
					});
				} else {
					transceiver = this._pc.addTransceiver(track, { 
			...
			.then((offer) =>
			{
				...
				// 	console.log('asdf offer', offerSdp);
				// 	offer = { type: 'offer', sdp: offerSdp };
				// }

				console.log('asdf offer', offer);
				logger.debug(
			...
			.then(() =>
			{
				...
				console.log('asdf localSdpObj', localSdpObj);
				...
				console.log('asdf answer', answer);
			...
			.then(() =>
			{
				...
				sdpUnifiedPlanUtils.fillRtpParametersForTrack(
					...
					localSdpObj,
					track,
					{ mid: transceiver.mid, planBSimulcast: false });

				console.log('asdf params', rtpParameters);
	...
	addConsumer(consumer)
	{
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				...
				console.log('asdf offer', remoteSdp);
				logger.debug(
